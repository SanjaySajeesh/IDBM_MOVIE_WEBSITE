...
public class LoginController {
    ...
    public static Handler handleLoginPost = ctx -> {
        ...
        if (!UserController.authenticate(getQueryUsername(ctx), getQueryPassword(ctx))) {
            ...
            ctx.sessionAttribute("currentUserType", UserController.getType(getQueryUsername(ctx)));
            model.put("authenticationSucceeded", true);
            model.put("currentUser", getQueryUsername(ctx));
            model.put("currentUserType", ctx.sessionAttribute("currentUserType"));
   ...
   public static Handler ensureLoginBeforeViewing = ctx -> {
        if (!ctx.path().startsWith("/admin") && !ctx.path().startsWith("/submit_show")) {
            return;
        }
